...
export async function courseLoader() {
  ...
  try {
    return axios
      .get("http://localhost:5000/courses/enrollment/" + sid)
      .then((res) => res.data)
      .then((response) => {
        return response;
      })
...
async function enroll(courseid, setRefresh) {
  const sid = localStorage.getItem("loggedIn");
  console.log(courseid, sid);
  try {
    return axios.post("http://localhost:5000/courses/enroll/", { courseid, sid })
      .then((res) => res.data)
      .then((response) => {
  ...
  return (
    <div key={i}
      ...
      <div className="flex justify-between">
        ...
        <button onClick={() => enroll(course.courseid, setRefresh)} type="button" className="w-1/6 focus:outline-none text-white bg-green-600 hover:bg-green-800 font-medium rounded-lg text-sm px-5 py-2.5 me-2 my-3">Enroll</button>
      </div>
    </div>
  );
}
